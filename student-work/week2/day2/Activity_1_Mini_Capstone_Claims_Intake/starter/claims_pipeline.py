...
def load_clean_claims(path):
    ...
    with open(path, newline="") as f:
        reader = csv.DictReader(f) #column name is key, and column values are values 
        for row in reader:
            ...
            reserve = (row.get("reserve")).strip()
            paid = (row.get("paid")).strip()
            if not claim_id or not reserve or not paid: #reserve is NONE 
                rejected++
            else:
                clean.append(row)
